...
export default function RecordTab() {
    ...
    const { recording } = await Audio.Recording.createAsync(
      ...
      (status) => {
        if (status.isRecording && status.metering !== undefined) {
          setAmplitude(Math.max(0, (status.metering + 60) / 60));
        }
      },
      100,
    );
  ...
  const stopAndUpload = useCallback(async () => {
    ...
    formData.append("file", { uri, name: `take_${Date.now()}.m4a`, type: "audio/m4a" } as unknown as Blob);
  ...
  const meterFlex = Math.max(0.001, amplitude);
  ...
  return (
    <SafeAreaView className="flex-1 bg-ink px-6">
      <View className="flex-1 items-center justify-center gap-8">
        ...
        {phase === "recording" && (
          <>
            <Body className="text-red-400">Recording…</Body>
            <Pressable
        ...
        {phase === "done" && (
          <>
            ...
            <Pressable onPress={reset} className="px-6 py-3 rounded-full bg-gold">
